...
const EventPage = async ({ params }: {params: Promise<{ ksuid: string }>}) => {  
  ...
  return <div className='min-h-screen flex flex-col bg-de-background-dark text-white'>
      ...
      <main className='w-full flex flex-col items-center flex-1'>
        <Event fallbackData={serverData} org={org} eventKsuid={ksuid} />
        {org.account_id && <p className='text-sm mt-4 hidden'>Stripe Account: {org.account_id} : {theme} {JSON.stringify(orgData,null,2)}</p>}
      </main>
      <DanceEngineFooter/>
    </div>
}
