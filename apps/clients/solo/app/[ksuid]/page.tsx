...
import EventExperience from '../../components/event-experience/EventExperience';
...
const EventPage = async ({
  ...
  if (useV2) {
    ...
    const orgRes = await fetch(orgApiUrl, {
      next: { revalidate: 120, tags: [format(new Date(), 'yyyy-MM-ddTHH:mm:ss.SSSxxx')] }
    });
    ...
    const orgsRes = await fetch(orgsApiUrl, { cache: 'force-cache', next: { revalidate: 120, tags: [format(new Date(), 'yyyy-MM-ddTHH:mm:ss.SSSxxx')] } });
    const orgsData = await orgsRes.json() as { organisations: OrganisationType[] };
    const orgDetails = orgsData.organisations.filter((orgCheck: OrganisationType) => orgCheck.organisation && orgCheck.organisation === orgSlug);
    org = orgDetails[0] || {
      name: 'Unknown Organisation',
      slug: 'unknown-org',
      description: '{"type":"doc","content":[{"type":"paragraph","content":[{"type":"text","text":"No organisation found for this domain"}]}]}'
    ...
    return (
      <div className='min-h-screen'>
        <main className='w-full flex flex-col items-center flex-1'>
          <EventExperience fallbackData={serverData} org={org as OrganisationType} eventKsuid={ksuid} />
          {('account_id' in org) && org.account_id ? <p className='text-sm mt-4 hidden'>Stripe Account: {org.account_id}</p> : null}
        </main>
      </div>
    );
  }
  ...
  return <div className='min-h-screen flex flex-col bg-de-background-dark text-white'>
      ...
      <main className='w-full flex flex-col items-center flex-1'>
        <Event fallbackData={serverData} org={org as OrganisationType} theme={theme} eventKsuid={ksuid} />
        {('account_id' in org) && org.account_id ? <p className='text-sm mt-4 hidden'>Stripe Account: {org.account_id}</p> : null}
      </main>
      <DanceEngineFooter/>
    </div>;
