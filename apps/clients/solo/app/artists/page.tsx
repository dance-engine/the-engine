      ...
      <main className='w-full justify-center' >
        <div className=' w-full px-4 lg:px-0 flex justify-center border-t-6' style={{backgroundColor: 'var(--alternate-bg-color)', borderColor: 'var(--highlight-color)'}}>
          <div className={`max-w-4xl w-4xl px-4 lg:px-0 mb-12 text-white text-xl `}> 
              ...
              <div className='sm:flex flex-wrap gap-3 justify-center'>
                 {Object.keys(artists).sort().map((artistKey) => (
                <div key={artistKey} className='mb-2 rounded w-72'>
                  <div id="hero" className={`w-full min-h-[200px] rounded-t-md text-white flex items-stretch justify-center bg-no-repeat \
                  ${theme == 'latin-soul' ?  "bg-cover bg-center": "bg-cover bg-top sm:bg-center md:bg-top sm:bg-size-[1400px_auto] md:bg-cover"}`} 
                  style={{ backgroundImage: `url(${artists[artistKey]?.image || '/artists/default.jpg'})` }}
                ></div>
                  <Link className="text-2xl bg-[var(--highlight-color)] text-shadow-neutral-900 text-shadow-2xs px-3 py-1 hover:underline block rounded-b-md w-full " href={`/artists/${artistKey}`}>{artists[artistKey].name}</Link>
                </div>
              ) )}
              </div>
