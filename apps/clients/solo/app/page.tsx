  ...
  return <div className='w-full' style={{ backgroundColor: 'var(--main-bg-color)', color: 'var(--main-text-color)' }}>
      ...
      <header className='w-full bg-black text-white flex justify-center'>
        <div className='max-w-4xl w-4xl px-4 uppercase font-black lg:px-0 py-3 flex items-center justify-center'>
          <a href="#" className='block'>{org.logo ? <img src={org.logo} alt={org.name} className='max-w-48 w-48'/> : org.name}</a>
        </div>
      </header>

      <main className='w-full justify-center' >

        <div id="hero" className='w-full h-[60vh] XXmin-h-[550px] max-h-[1024px] text-white flex items-stretch justify-center bg-no-repeat bg-center md:bg-auto bg-size-[1400px_auto]' style={{ backgroundImage: `url(${org.banner})` }}>
        ...
        <div>
           <div className={`max-w-4xl px-4 lg:px-0 py-12 flex flex-col items-center\ `}>
            <h2 className='text-3xl font-bold mb-4'>Early Bird Ticket</h2>
            <p className='mb-6'>We have a limited amount of early bird discounted tickets at only £40</p>
            <button className='rounded px-4 py-2 text-2xl' style={{backgroundColor: 'var(--highlight-color)'}}>Get yours now!</button>
           </div>
        </div>
