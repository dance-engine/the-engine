...
} from "@dance-engine/ui/PassSelectorContext";

export default function BundleCard({
  bundleData,
  event,
}: {
  bundleData: BundleTypeExtended;
  ...
  const saving = bundle.includes
    ? bundle.includes
        .map((item_id) => event?.items?.[item_id])
        .reduce((acc, item) => {
          return acc + (item?.primary_price || 0);
        }, 0) - bundle.primary_price
    : 0;
  const isIncluded =
    included &&
    included.length > 0 &&
    new Set(bundle.includes).isSubsetOf(
      new Set([...included.flat(), ...selected]),
    );
  const isSelected =
    selected && selected.length > 0 && selected.includes(bundle.ksuid);
  const ignoreToggle = (
    bundle: BundleTypeExtended,
    items: Record<string, ItemType>,
  ) => {
    console.debug(`ignore ${bundle.name} toggle, ${items}`);
  };
  ...
  return (
    <Fragment>
      <div
        onClick={() => {
          (isIncluded && !isSelected ? ignoreToggle : toggleBundle)(
            bundleData,
            event.items || {},
          );
        }}
      ...
      >
        <header className="px-4 py-5 sm:p-6 flex-1 bg-keppel-on-light/90 ">
          ...
          <p>
            {bundle.current_price_name()}: {bundle.current_price()}
          </p>
          {/* <p>{bundle.ksuid}</p> */}
          {bundle.includes && (
            <div>
              ...
              <ul className="flex gap-1">
                {bundle.includes
                  .filter((item_id) => event?.items?.[item_id])
                  .map((item_id) => {
                    return (
        ...
        {(isSelected || isIncluded) && (
          <div
            className={`overflow-hidden absolute top-0 h-full w-full bg-black/70 ${isSelected ? "text-pear-logo font-bold" : "text-white"}  text-3xl flex flex-col items-center justify-center`}
          >
