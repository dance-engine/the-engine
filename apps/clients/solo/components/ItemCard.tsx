...
export default function ItemCard({ itemData }: { itemData: ItemType }) {
  ...
  const inDirect =
    selected && selected.length > 0 && selected.includes(itemData.ksuid);
  const inCart = inABundle || inDirect;
  const ignoreToggle = (ignore: ItemType) => {
    console.log(`ignore ${ignore} toggle`);
  };

  return (
    <div
      onClick={() => {
        (inDirect ? toggleItem : inABundle ? ignoreToggle : toggleItem)(
          itemData,
        );
      }}
    ...
    >
      <header className="px-4 py-5 sm:p-6 bg-cerise-on-light/90 ">
        ...
        {/* <p>{itemData.ksuid}</p> */}
        <p>
          {itemData.primary_price_name}: £
          {(itemData.primary_price / 100).toFixed(2)}
        </p>
      </header>
      ...
      {inCart && (
        <div
          className={`${inDirect ? "text-pear-logo" : "text-white"} overflow-hidden absolute top-0 h-full w-full bg-black/70 text-2xl flex flex-col items-center justify-center`}
        >
