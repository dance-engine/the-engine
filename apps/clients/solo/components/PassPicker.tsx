...
import { PassSelectorProvider } from "@dance-engine/ui/PassSelectorContext";
...
const PassPicker = ({
  ...
  return (
    <PassSelectorProvider event={event} org={org}>
      <div
        className={`grid grid-cols-1 max-w-full m-auto \
      ...
      >
        {event.bundles &&
          event.bundles.map((bundle: BundleTypeExtended) => (
      ...
      >
        {event &&
          ...
          Object.keys(event.items).map((item_ksuid: string) =>
            event.items?.[item_ksuid] ? (
              <ItemCard key={item_ksuid} itemData={event.items[item_ksuid]} />
            ) : null,
          )}
      </div>

      <div className="hidden">
        <PassDebug event={event} />
      </div>
