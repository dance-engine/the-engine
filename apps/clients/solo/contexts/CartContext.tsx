...
export const CartSelectorContext =
  createContext<CartSelectorState>(initialSelection);

type Action = { type: "add"; items: ItemType[] };
...
export function CartProvider({
  ...
  const lineItems = useCallback(() => {
    ...
    const returnVal = [...items, ...bundles].filter(Boolean) as (
      | ItemType
      | BundleTypeExtended
    )[];
    return returnVal;
  ...
  const cartAmount = lineItems().reduce(
    (total, item) => total + (item?.primary_price || 0),
    0,
  );
  ...
  return (
    <CartSelectorContext.Provider value={state}>
      ...
      <h2 className="mt-6 text-xl">
        Total Cost: £{(cartAmount / 100).toFixed(2)}
      </h2>
      ...
      {lineItems() ? (
        <StripeMultiPurchaseButton
          accountId={
            org
              ? org.account_id || "acct_1Ry9rvDqtDds31FK"
              : "acct_1Ry9rvDqtDds31FK"
          }
          org={org}
          // priceIds={priceIds}
          lineItems={lineItems()}
          ...
          label="Checkout Now"
          className="text-de-background-dark px-6 py-3 bg-pear-logo rounded text-xl mt-6"
        />
      ) : (
        "Nothing to add"
      )}
