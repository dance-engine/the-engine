...
const CustomPage = () => {
  ...
  const { activeOrg, switchOrg, availableOrgs } = useOrgContext();
  ...
  if (!isSignedIn) {
    ...
    return (
      <div className="flex items-center gap-x-4 lg:gap-x-6">
        <SignInButton>
          <button
            className="block px-3 py-1 text-gray-900"
            role="menuitem"
            tabIndex={-1}
            id="user-menu-item-1"
          >
      ...
      {sitesAdmind.includes("*") ? (
        <div>
          {summaryData &&
            ...
            summaryData.organisations.map((site: Record<string, string>) => {
              ...
              return (
                <div key={`site-${orgSlug}-api`} className="pt-3">
                  <div className="flex justify-between items-start">
                    <div>
                      <h2 className="text-lg">
                        {nameFromHypenated(site.name || "Unknown")}{" "}
                        {active ? <Badge>active</Badge> : null}{" "}
                      </h2>
                    </div>
                    {active ? null : (
    ...
    <div className="flex justify-center">
      <UserProfile>
        <UserProfile.Page
          label="Permissions"
          url="permissions"
          labelIcon={<BiSolidLockOpen className="w-4 h-4" />}
        >
          <CustomPage />
        </UserProfile.Page>
