...
def create_stripe_catalog(organisation: str, event_id: str, item, account_id, stripe):
    """
    """
...
def rollback_stripe_created(created_list: list[StripeCreated], account_id, stripe) -> None:
    for created in reversed(created_list):
        # archive prices
        for pid in created.price_ids:
            try:
                stripe.Price.modify(pid, active=False, stripe_account = account_id)
        ...
        if created.product_id:
            try:
                if len(created.price_ids) == 0:   # only possible if no prices
                    stripe.Product.delete(created.product_id, stripe_account = account_id)
                else:
                    stripe.Product.modify(created.product_id, active=False, stripe_account = account_id)
            except Exception as e:
