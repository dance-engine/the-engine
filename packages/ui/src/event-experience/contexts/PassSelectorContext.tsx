...
export const PassSelectorContext =
  createContext<PassSelectorState>(initialSelection);

type ActionsAPI = {
  toggleBundle: (
    bundle: BundleTypeExtended,
...
export const PassSelectorActionsContext = createContext<ActionsAPI | undefined>(
  undefined,
);
...
function reducer(state: PassSelectorState, action: Action): PassSelectorState {
  ...
  switch (action.type) {
    case "toggleBundle": {
      ...
      const includedIdx = state.included.findIndex((arr) =>
        incEquals(arr, includes),
      );
      ...
      if (present) {
        const newState = {
          ...
          includedPrices: state.includedPrices.filter(
            (_, i) => i !== includedIdx,
          ),
      ...
      const newState = {
        ...
        selectedPrices: [
          ...
          getPriceInCents(action.bundle, state.pricingTier),
        ],
        included: [...state.included, includes],
        includedPrices: [...state.included, includes].map((group) => {
          return group.map((itemKsuid) =>
            getPriceInCents(items[itemKsuid], state.pricingTier),
          );
        }),
        event: state.event || null,
        org: state.org || null,
    ...
    case "toggleItem": {
      ...
      const newState = {
        ...
        selectedPrices: [
          ...state.selectedPrices,
          getPriceInCents(action.item, state.pricingTier),
        ],
        included: state.included,
        includedPrices: state.includedPrices,
        event: state.event || null,
...
export function PassSelectorProvider({
  event,
  org,
  children,
}: {
  ...
  const toggleBundle = useCallback(
    (bundle: BundleTypeExtended, items: Record<string, ItemType>) => {
...
export function usePassSelectorState() {
  ...
  if (!ctx)
    throw new Error(
      "usePassSelectorActions must be used within PassSelectorProvider",
    );
  return ctx;
}
...
const buildBestCombo = (selectorState: PassSelectorState) => {
  const itemsSpecific = selectorState.selected
    .map((ksuid, i) => {
  ...
  const itemsFromBundles = Array.from(
    new Set(
      selectorState.included
        .map((group, gIdx) => {
          return group.map((_, iIdx) => {
            ...
            return {
              ...
              ksuid: _,
              primary_price:
                getPriceInCents(item, selectorState.pricingTier) ||
                selectorState.includedPrices[gIdx]?.[iIdx] ||
                0,
            } as unknown as ItemType & { type: string };
          });
        })
  ...
  const requestedItems = [
    ...new Map(
      [...itemsSpecific, ...itemsFromBundles].map((item) => [item.ksuid, item]),
    ).values(),
  ];
  ...
  const bestState = {
    selected: [
      ...suggestedItems.chosenBundles.map((bundle) => bundle.ksuid),
      ...suggestedItems.chosenItems.map((item) => item.ksuid),
    ],
    selectedTypes: [
      ...suggestedItems.chosenBundles.map(() => "bundle"),
      ...suggestedItems.chosenItems.map(() => "item"),
    ],
    selectedPrices: [
      ...suggestedItems.chosenBundles.map((bundle) => bundle.primary_price),
      ...suggestedItems.chosenItems.map((item) => item.primary_price),
    ],
    included: Array.from(
      new Set([
        ...suggestedItems.chosenBundles.map((bundle) => bundle.includes),
      ]),
    ),
    includedPrices: [
      ...suggestedItems.chosenBundles.map((bundle) =>
        bundle.includes.map((item_ksuid) =>
          getPriceInCents(
            selectorState.event?.items?.[item_ksuid],
            selectorState.pricingTier,
          ),
        ),
      ),
    ],
