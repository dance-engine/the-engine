...
const TextInput: React.FC<TextInputProps> = ({ label, name, register, validate, error, fieldSchema }) => (
  <CustomComponent label={label} name={name} htmlFor={name} error={error} fieldSchema={fieldSchema}>
    <input {...register(name)} type="text" name={name} id={name} placeholder={fieldSchema._def.placeholder ?? ""} onBlur={validate} className="border p-2 rounded-md border-gray-300" />
  </CustomComponent>
);
