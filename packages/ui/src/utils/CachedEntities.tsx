    ...
    <div {...(className && { className })}>
      <div className="w-full overflow-auto block ">
        <table className='block md:table w-full'>
          ...
          {entities.filter((enty: Record<string, unknown>)=>enty.name ? true : false).map((enty: Record<string, unknown>) => {
            return (
              <tr key={enty.ksuid as string} className='grid md:table-row  grid-col-1 gap-3 border border-gray-300 p-3 rounded bg-gray-50'>
                <td className='block md:table-cell p-2'>
                  ...
                  <div className='text-2xl leading-none text-gray-800'>{enty.name as string}</div>
                </td>
                <td className='block md:table-cell'>
                  ...
                  <div className='text-lg text-gray-800'>{enty.start_date as string}</div>
                </td>
